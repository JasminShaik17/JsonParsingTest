package cubex.mahesh.jsonparsingtest
...
class MainActivity : AppCompatActivity() {
    ...
    fun insert(v:View)
    {
     var e = Employee( );
      e.id = et1?.text.toString().toInt()
        ...
        var os = openFileOutput("employees.json",
                Context.MODE_PRIVATE)
        os.write(json_data.toByteArray())
        os.flush()
        os.close()
    ...
    var emps =  g.fromJson<Employees>(isr,
             Employees::class.java)
     var list =    emps.employees
      var temp_list = ArrayList<String>( )
        for( x in list){
            temp_list.add(x.id.toString()+
                    "\t"+x.name+"\t"+x.desig+"\t"+x.dept)
        }
